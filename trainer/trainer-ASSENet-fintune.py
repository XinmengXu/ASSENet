...
from trainer.base_trainer_ASSENet import BaseTrainer
    ...
    def _train_epoch(self, epoch):
        ...
        with tqdm(total = num_batchs) as pbar:
            for i, (mixture, clean, name) in enumerate(self.train_data_loader):
                ...
                enhanced_cpl, enhanced, probs, probs1, probs2, distr_1, distr_2, distr_3 = self.model(mixture)
                reward_1 = get_reward(probs)
                reward_2 = get_reward(probs1)
                reward_3 = get_reward(probs2)

				
                advantage_1 = reward_1.mean()
                advantage_2 = reward_2.mean()
                advantage_3 = reward_3.mean()


                loss1 = distr_1.log_prob(probs).sum() * Variable(advantage_1)
                loss2 = distr_2.log_prob(probs1).sum() * Variable(advantage_2)
                loss3 = distr_3.log_prob(probs2).sum() * Variable(advantage_3)

                loss = self.model.loss(enhanced, clean, mode='SiSNR') + 0.01 * (loss1 + loss2 + loss3)/3
